 ...
 * 然后通过Header[zuultoken]取得EncryptDecryptUtil.deflate2Base62Encrypt值进行解码后进行 IP 验证。
 ...
 * @since Aug 11, 2016
 */
public class ShiroGatewayAccessControlFilter extends AdviceFilter implements OnceValidAdvice {
    ...
    public boolean preHandle(ServletRequest servletRequest, ServletResponse servletResponse) throws Exception {
        ...
        /**
         * 验证： 如果Header[zuultoken]采用了约定的加密方式。
         */
        String zuulValue = request.getHeader("zuultoken");
        ...
        try {//开启shiro鉴权
            ...
            Assert.state(ZuulTokenHelper.validate(zuulValue, "zuul@admin.com"), "Shiro zuul auth failed, remote ip not register in eureka!");
            return true;
        } catch (Exception e) {
